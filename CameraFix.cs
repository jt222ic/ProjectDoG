...
public class CameraFix : MonoBehaviour {
    ...
    public Transform thisTransform; // camera Transform
    ...
    void Update()
    {
        ...
        if (cameraFollowX)
        {
            if (Mathf.Abs(cameraTarget.transform.position.x - thisTransform.position.x) > threshold)
            {
                ...
                thisTransform.position = new Vector3((cameraTarget.transform.position.x - thisTransform.position.x) * fraction, transform.position.y, transform.position.z);
            }
        }
